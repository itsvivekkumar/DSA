...
public class FloorAndCeil_2 {
    ...
    //Floor means greatest ele b/w collection of all smaller elements than target[ just sort array if not]
    public static void floorBS(int[] arr, int tarF) {     //5.3, 5.7 = floor(5)
    ...
    //Ceil means smallest ele b/w collection of all greater elements than target
    public static void ceilBS(int[] arr, int tarC) {    //5.3, 5.7 = ceil(6)
        int s = 0, e = arr.length - 1;
        int res = 0;
        while (s <= e) {
            int mid = s + (e - s) / 2;
